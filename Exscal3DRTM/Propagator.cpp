	...
	for(ishot=shots;ishot<shote;ishot++){
		...
		for(k=0;k<nt;k++)
		{
			...
			if(d_phi==NULL||d_the==NULL)
				...
				hipLaunchKernelGGL(VTI_forward_wavefield_update_2d, dim3(grid2new), dim3(block2new), 0, 0,d_c,d_c2, u1,u2, nx, ny, nz,1.0f/dx,1.0f/dy,1.0f/dz,dt,wavelet[k],d_vp,d_epsilon, d_delta,sx, sy, sz,d_pmlx,d_pmly,d_pmlz );
			else
				hipLaunchKernelGGL(TTI_forward_wavefield_update, dim3(grid), dim3(block), 0, 0, u1,u2,d_c,d_c2, nx, ny, nz,dx,dy,dz,dt,wavelet[k],d_vp,d_epsilon, d_delta,d_phi,d_the,sx, sy, sz,ux,uy,uz,d_pmlx,d_pmly,d_pmlz );
		...
		for(k=nt-1;k>=0;k--){
			...
			if(mode==0)
				...
				hipLaunchKernelGGL(VTI_backward_wavefield_update_2d, dim3(grid2new), dim3(block2new), 0, 0,d_c,d_c2, u1,u2, nx, ny, nz,1.0f/dx,1.0f/dy,1.0f/dz,dt,0,d_vp,d_epsilon, d_delta,d_pmlx,d_pmly,d_pmlz );
			else
				hipLaunchKernelGGL(TTI_backward_wavefield_update, dim3((grid)), dim3((block)), 0, 0,  u1,u2,d_c,d_c2, nx, ny, nz,dx,dy,dz,dt,0,d_vp,d_epsilon, d_delta,d_phi,d_the,sx, sy, sz,ux,uy,uz,d_pmlx,d_pmly,d_pmlz	);
			...
			if(d_phi==NULL||d_the==NULL)
				...
				hipLaunchKernelGGL(VTI_restruction_wavefield_update_2d, dim3(grid2new), dim3(block2new), 0, 0,d_c,d_c2, fu1,fu2, nx, ny, nz,1.0f/dx,1.0f/dy,1.0f/dz,dt,0,d_vp,d_epsilon, d_delta,d_pmlx,d_pmly,d_pmlz );
			else
				hipLaunchKernelGGL(TTI_reconstruction_wavefield_update, dim3((grid)), dim3((block)), 0, 0,  fu1,fu2,d_c,d_c2, nx, ny, nz,dx,dy,dz,dt,0,d_vp,d_epsilon, d_delta,d_phi,d_the,sx, sy, sz,fux,fuy,fuz,d_pmlx,d_pmly,d_pmlz);
			//EX-back:exchange
			float* tmp=u1;
			u1 = u2;
			u2 = tmp;
...
__global__ void VTI_forward_wavefield_update_2d(
	...
	for(int idx=M;idx<nx-M;idx++){
		...
		if(threadIdx.x<M)
			//uS[threadIdx.y+M][threadIdx.x   ]=tex1Dfetch(u1_tex,idx*ny*nz+idy*nz+idz-M);
			uS[threadIdx.y+M][threadIdx.x   ]=u1[idx*ny*nz+idy*nz+idz-M];
		if(threadIdx.x>=blockDim.x-M)
			//uS[threadIdx.y+M][threadIdx.x+2*M]=tex1Dfetch(u1_tex,idx*ny*nz+idy*nz+idz+M);
			uS[threadIdx.y+M][threadIdx.x+2*M]=u1[idx*ny*nz+idy*nz+idz+M];
		if(threadIdx.y<M)
			//uS[threadIdx.y   ][threadIdx.x+M]=tex1Dfetch(u1_tex,idx*ny*nz+(idy-M)*nz+idz);
			uS[threadIdx.y   ][threadIdx.x+M]=u1[idx*ny*nz+(idy-M)*nz+idz];
		if(threadIdx.y>=blockDim.y-M)
			//uS[threadIdx.y+2*M][threadIdx.x+M]=tex1Dfetch(u1_tex,idx*ny*nz+(idy+M)*nz+idz);
			uS[threadIdx.y+2*M][threadIdx.x+M]=u1[idx*ny*nz+(idy+M)*nz+idz];
		uS[threadIdx.y+M][threadIdx.x+M]=xval[M];

		__syncthreads();

		//float eps=tex1Dfetch(eps_tex,index);
		//float vp=tex1Dfetch(vp_tex,index);
		...
		float S2=(((1.0f+2.0f*eps)*(Ux * Ux + Uy * Uy) + Uz * Uz +EPS)*((1.0f+2.0f*eps)*(Ux * Ux + Uy * Uy) + Uz * Uz +EPS));
		S2=(Ux*Ux + Uy*Uy)*(Uz*Uz)/S2;
		//float	S = 0.5f*(1.0f+sqrtf(1.0f-8.0f*(eps-tex1Dfetch(delta_tex,index))*S2));
		float	S = 0.5f*(1.0f+sqrtf(1.0f-8.0f*(eps-d_delta[index])*S2));
		...
		//float taper=1.0f/(1.0f+ tex1Dfetch(pmlx_tex,idx)+ tex1Dfetch(pmly_tex,idy)+ tex1Dfetch(pmlz_tex,idz));//absorb layer coefficient
